 ...
 * CHDFS {@link ChdfsInodeAttributesProvider}.
 */
public final class ChdfsInodeAttributesProviderFactory implements InodeAttributesProviderFactory {
  private static final Logger LOG = LoggerFactory.getLogger(
          ChdfsInodeAttributesProviderFactory.class);
  private URI mUri;
  ...
  public InodeAttributesProvider create(String path, @Nullable UnderFileSystemConfiguration conf) {
    ...
    return new ChdfsInodeAttributesProvider(mUri);
  }

  @Override
  public boolean supportsPath(String path, @Nullable UnderFileSystemConfiguration conf) {
    LOG.info("ChdfsInodeAttributesProviderFactory.supportsPath for path: {}", path);
    boolean isChdfs = ServerConfiguration.getList(TxPropertyKey.UNDERFS_CHDFS_PREFIXES).stream()
        .anyMatch(path::startsWith);
    return conf != null && AuthorizationPluginConstants.AUTH_VERSION.equalsIgnoreCase(
            conf.getString(TxPropertyKey.UNDERFS_SECURITY_AUTHORIZATION_PLUGIN_NAME)) && isChdfs;
  }
}

...
import alluxio.master.file.uritranslator.CompositeUriTranslator;
...
public class ShimFsCompositeUriTranslatorIntegrationTest {
  ...
  public LocalAlluxioClusterResource mLocalAlluxioClusterResource =
      new LocalAlluxioClusterResource.Builder()
          ...
          .setProperty(TxPropertyKey.MASTER_URI_TRANSLATOR_IMPL,
              CompositeUriTranslator.class.getName())
          .setProperty(TxPropertyKey.MASTER_COMPOSITE_URI_TRANSLATOR_IMPL,
              "s3a://=" + MergeAuthorityToPathUriTranslator.class.getName()
                  + ",file://=" + AutoMountUriTranslator.class.getName())
          .build();
  ...
  @Test
  public void listFilesThroughLocalScheme() throws Exception {
    int testFileCount = 10;
    ...
    for (int i = 0; i < testFileCount; i++) {
      ...
      mShimFileSystem.createDirectory(foreignUri, createDirectoryPOptions);
    }

    // List files via shim-fs.
    List<URIStatus> shimStatusList = mShimFileSystem.listStatus(
        new AlluxioURI(foreignRoot),
        ListStatusPOptions.newBuilder().setRecursive(true).build());
    Assert.assertEquals(testFileCount, shimStatusList.size());
    // List directories via Alluxio-fs.
    // Get foreign root status to get Alluxio path.

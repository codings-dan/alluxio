  ...
  private void createUfsBlock(BlockWriteRequestContext context)
    ...
    if (mUserInfo.getAuthorizedUserName() != null) {
      context.setCounter(MetricsSystem.counterWithTags(counterKey.getName(), counterKey
          .isClusterAggregated(), MetricInfo.TAG_UFS, ufsString, MetricInfo.TAG_USER,
          mUserInfo.getAuthorizedUserName()));
      context.setMeter(MetricsSystem.meterWithTags(meterKey.getName(), meterKey
          .isClusterAggregated(), MetricInfo.TAG_UFS, ufsString, MetricInfo.TAG_USER,
          mUserInfo.getAuthorizedUserName()));
    } else {
      context.setCounter(MetricsSystem.counterWithTags(counterKey.getName(), counterKey
          .isClusterAggregated(), MetricInfo.TAG_UFS, ufsString));
      context.setMeter(MetricsSystem.meterWithTags(meterKey.getName(), meterKey
          .isClusterAggregated(), MetricInfo.TAG_UFS, ufsString));
    }
  }
